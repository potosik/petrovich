...
namespace Petrovich.Web.Tests
{
    public class LocalizationTests
    {
        ...
        [Fact]
        public void TestForMissingOrDuplicatePriceCalculationTypeResourceStrings()
        {
            var enumNames = EnumUtils.GetValues<PriceCalculationTypeBusiness>().ToArray();
            var enumValues = new List<string>();
            var resManager = new System.Resources.ResourceManager("Petrovich.Web.Properties.Resources", typeof(Startup).Assembly);
            ...
            for (var index = 0; index < enumNames.Length; index++)
            {
                ...
                var res = resManager.GetString($"PriceCalculationType_{enumName}");
                if (res == null)
                {
        ...
        [Fact]
        public void TestForMissingOrDuplicatePriceCalculationTypeAbrResourceStrings()
        {
            var enumNames = EnumUtils.GetValues<PriceCalculationTypeBusiness>().ToArray();
            var enumValues = new List<string>();
            var resManager = new System.Resources.ResourceManager("Petrovich.Web.Properties.Resources", typeof(Startup).Assembly);
            ...
            for (var index = 0; index < enumNames.Length; index++)
            {
                ...
                var res = resManager.GetString($"PriceCalculationType_Abr_{enumName}");
                if (res == null)
                {
                    brokenPriceTypes.Add(enumName);
                }
